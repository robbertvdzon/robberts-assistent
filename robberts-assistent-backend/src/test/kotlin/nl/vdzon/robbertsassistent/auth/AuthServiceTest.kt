...
class AuthServiceTest {
    ...
    @Test
    fun `requireAuthorization skips Google login entirely when preview bypass is enabled`() {
        val previewAuthService = AuthService(
            secrets.copy(previewSkipGoogleAuth = true),
            tokens.verifier(clientId),
        )

        // Geen header nodig, geen geldig token nodig: preview-omgevingen slaan Google-login volledig over.
        assertEquals("preview@robberts-assistent.local", previewAuthService.requireAuthorization(null))
    }
}
