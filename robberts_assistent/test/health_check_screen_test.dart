...
Widget _wrap(ApiClient api) => MaterialApp(
  home: Scaffold(body: HealthCheckScreen(api: api)),
);

void main() {
  testWidgets(
    'toont per onderdeel een kop met de ruwe statusregel(s) in selecteerbare tekst',
    (tester) async {
      final api = _FakeApiClient()
        ..sections = const [
          BriefingSection(
            ...
            items: [
              BriefingItem(
                text: 'huidig vermogen=100 W, gisteren opgewekt=12 kWh.',
                heading: 'Zonnepanelen',
              ),
              BriefingItem(
                text: '(nog geen koppeling, placeholder) geen fouten gemeld.',
                heading: 'Backups',
              ),
              BriefingItem(
                text:
                    'gezond=true, versie=4.16.3, beschikbare update=geen, gedegradeerde operators=geen.',
                heading: 'OpenShift',
              ),
              BriefingItem(
                text:
                    'Robotmaaier Maaier: activiteit=MAAIT, status=OK, errorCode=0, verbonden=true.',
                heading: 'Robotmaaier',
              ),
              BriefingItem(
                text: 'geen lopende of error-stories.',
      ...
      expect(
        find.textContaining('AI-samenvatting die niet getoond moet worden'),
        findsNothing,
      );
  ...
  testWidgets('toont de updatedAt-tijdstip van de (gecachete) systeemstatus', (
    tester,
  ) async {
  ...
  testWidgets(
    'reload-knop roept refreshHealthCheck aan en toont tijdens het laden een spinner',
    (tester) async {
  ...
  testWidgets('toont een melding als er geen systeemstatus-sectie is', (
    tester,
  ) async {
  ...
  testWidgets(
    'toont een foutmelding als het ophalen van de systeemstatus faalt',
    (tester) async {
