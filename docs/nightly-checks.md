...
- **Samenvatting**: `SummaryService` (`GET /api/v1/summary`) neemt de nightly-check-resultaten
  ...
  kunnen zijn. De systeemstatus-sectie (nog steeds live, niet
  `NightlyCheckRepository`-historie) staat in ruwe per-check-vorm op "Health check" onder
  "Meer", terwijl de briefing zonder systeemstatus op de hoofdtab "Vandaag" staat. Zie root
  `CLAUDE.md` §4 (`briefing`-rij) voor details.
