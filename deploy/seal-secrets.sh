...
EOF
  ...
  while IFS= read -r line || [[ -n "$line" ]]; do
    ...
    case "$key" in
      ...
      RA_GOOGLE_CLIENT_ID|RA_REMEMBER_SECRET|RA_ALLOWED_EMAILS|RA_PREVIEW_SKIP_GOOGLE_AUTH|RA_DATABASE_URL|RA_OPENAI_API_KEY|RA_TELEGRAM_BOT_TOKEN|RA_TELEGRAM_CHAT_ID|RA_GOOGLE_OAUTH_CLIENT_ID|RA_GOOGLE_OAUTH_CLIENT_SECRET|RA_GOOGLE_OAUTH_REFRESH_TOKEN|RA_FIREBASE_PROJECT_ID|RA_FIREBASE_DATABASE_ID|RA_FIREBASE_STORAGE_BUCKET|RA_HUSQVARNA_APP_KEY|RA_HUSQVARNA_APP_SECRET|RA_STRAVA_CLIENT_ID|RA_STRAVA_CLIENT_SECRET|RA_STRAVA_REFRESH_TOKEN)
        printf '  %s: |-\n' "$key"
        printf '%s\n' "$val" | sed 's/^/    /'
        count=$((count + 1))
        ;;
