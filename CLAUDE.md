...
| `watches` | Langdurige zoekopdrachten: geauthenticeerde CRUD, Firestore/in-memory-opslag en één configureerbare poller. `WatchSchedule.isDue` begrenst kantoorurenchecks tot werkdagen 09:00–17:00 (Europe/Amsterdam), maximaal uurlijks, en dagelijkse checks tot eenmaal per lokale kalenderdag. `JdkWatchPageFetcher` zet begrensde server-HTML om naar platte tekst; een eigen tool-loze `watchChatClient` beoordeelt de instructie. Fouten worden `ONBEKEND` en later opnieuw geprobeerd; een eerste vondst deactiveert de opdracht en verstuurt optioneel één FCM-push met `data.type=watch`. |
