...
| `watches` | **Zoekopdrachten** (SF-1489): houd een webpagina in de gaten tot een in vrije tekst beschreven situatie zich voordoet (bv. "meld het als de aaltjes weer op voorraad zijn"). `Watch`-model (`title`, `url`, `instruction`, `frequency` = `KANTOORUREN`/`DAGELIJKS`, `pushOnFound`, `active`, `lastCheckedAt`, `lastStatus`, `found`, `lastError`), `WatchRepository`-poort met `FirestoreWatchRepository` (collectie `watches`, doc-id = watch-id) of `InMemoryWatchRepository` (`WatchRepositoryConfig`, zelfde patroon als `ReminderRepositoryConfig`). `WatchesService` doet CRUD + één check: pagina ophalen met de JDK-`HttpClient` (User-Agent + timeout, injecteerbaar via constructor-default), HTML → platte tekst via een eigen `htmlToPlainText` **binnen de module** (bewuste duplicatie: de variant in `assistant/ai/WindTools` is `internal` en `ModulithArchitectureTest` bewaakt de grens), daarna instructie + paginatekst naar de tool-loze `watchChatClient` (`WatchAiConfig`, patroon van `BriefingAiConfig`) met een vast antwoord van twee regels (`GEVONDEN`/`NIET GEVONDEN` + korte NL-statuszin). Het antwoord wordt defensief geparsed: niet-herkend of leeg ⇒ `found = false` met een neutrale/ruwe status, geen exception en geen push — daardoor is `RA_MOCK_AI`/preview deterministisch zonder eigen mock-schakelaar. Push alleen bij de omslag "niet gevonden" → "gevonden" én met `pushOnFound`: één `PushService.sendToAll(title, status, {"type": "watch"})`, waarna de watch op `active = false` gaat (geen herhaalde meldingen; hervatten kan handmatig). `WatchScheduler`: `@Scheduled(fixedDelayString = "\${ra.watches.poll-interval-ms:300000}")` met de pure, los testbare `isDue(...)` — KANTOORUREN = hoogstens één keer per klokuur op ma t/m vr in de uren 09:00 t/m 16:59 (Europe/Amsterdam), DAGELIJKS = hoogstens één keer per kalenderdag, gepauzeerd = nooit; elke check in een eigen `runCatching` zodat een netwerk-/HTTP-/AI-fout alleen `lastError` op díe watch zet (vorige status blijft staan) en de rest niet blokkeert. `WatchesController`: `GET`/`POST /api/v1/watches`, `PUT`/`DELETE /api/v1/watches/{id}`, `POST /api/v1/watches/{id}/check`, alles achter `authService.requireAuthorization(...)` (stijl `RemindersController`); pauzeren/hervatten loopt via `active` op de `PUT`. Leunt alleen op `firebase`, `push` en `auth`; geen nieuwe koppeling, secret of dependency. |
...
- **`robberts_assistent/`** — bottom-navigatie met 6 tabs (sinds SF-1489: Upcoming 0,
  Health check 1, Assistent 2, Herinneringen 3, **Zoekopdrachten 4**, Meer 5). Eerste tab is **"Upcoming"**
  ...
  18:00-FCM-push (`data['type'] == 'briefing'`) opent deze tab automatisch
  (`FcmService.deepLinkTab`, afgehandeld in `home_screen.dart`). Sinds SF-1267/SF-1268 staat er
  ernaast een tweede tab **"Health check"** (nieuw `health_check_screen.dart`): toont uitsluitend
  ...
  eigen cache: verversen op Health check raakt Upcoming's `updatedAt` niet en omgekeerd. Beide
  tabs worden daarnaast backend-side elk uur automatisch ververst (zie `briefing`-module hierboven).
  Daarnaast: chat-assistent met
  ...
  `more_screen.dart`. Sinds SF-1489 daarnaast de tab **"Zoekopdrachten"** (`watches_screen.dart`,
  index 4, vóór "Meer"): lijst met per zoekopdracht de titel, de laatste status, het laatste
  controlemoment + frequentie (en "gepauzeerd"), een eventuele foutregel en een groene markering
  bij gevonden; per rij "Nu controleren" (spinner op die rij), pauzeren/hervatten en verwijderen
  met bevestigingsdialoog, en aanmaken/bewerken via `WatchDialog` (titel, URL, instructie,
  frequentie, push-schakelaar) in de stijl van `schedules_screen.dart`. Praat met de
  `watches`-backendmodule via de `// -- Zoekopdrachten --`-sectie in `ApiClient` (`Watch`-model met
  `fromJson`, in de stijl van `Reminder`); een tik op een watch-push (`data['type'] == 'watch'`)
  opent deze tab via hetzelfde `FcmService.deepLinkTab`-mechanisme als de briefing-push.
  Google-login (web: GIS-knop, mobiel: `signIn()`). Web op OpenShift
...
Nieuw (SF-1489): **langdurige zoekopdrachten** ("watches") — laat de assistent een webpagina in de
gaten houden tot een in gewone taal beschreven situatie zich voordoet (bv. "meld het als de aaltjes
weer op voorraad zijn"). Nieuwe Spring-Modulith-module `watches` (model + `WatchRepository`-poort
met Firestore-collectie `watches` / in-memory-fallback, `WatchesService` met CRUD + één check,
tool-loze `watchChatClient` in `WatchAiConfig`, `WatchScheduler` en `WatchesController` op
`/api/v1/watches`, zie §4) die alleen op `firebase`, `push` en `auth` leunt — geen nieuwe
koppeling, geen nieuw secret, geen nieuwe dependency. De check haalt de pagina met de
JDK-`HttpClient` op (alleen server-side HTML, geen headless browser/JS-rendering — een pagina die
zijn status pas client-side opbouwt levert "kon niet bepalen" of een foutmelding, geen crash),
zet 'm met een eigen `htmlToPlainText` om naar platte tekst (afgetopt op ~6000 tekens) en laat de
AI in twee vaste regels oordelen (`GEVONDEN`/`NIET GEVONDEN` + statuszin), defensief geparsed.
Poller: `@Scheduled(fixedDelayString = "${ra.watches.poll-interval-ms:300000}")` met een pure
`isDue(...)` — KANTOORUREN = hoogstens één keer per klokuur ma t/m vr 09:00–16:59
(Europe/Amsterdam), DAGELIJKS = hoogstens één keer per kalenderdag, gepauzeerd = nooit. Push
uitsluitend bij de omslag niet-gevonden → gevonden en alleen met `pushOnFound`, daarna
`active = false` zodat er geen herhaalde meldingen komen. App-kant: nieuwe tab "Zoekopdrachten" op
index 4 (vóór "Meer", dat naar 5 schuift) — Upcoming blijft index 0, dus de bestaande
briefing-deep-link blijft werken — met `watches_screen.dart`, een eigen `ApiClient`-sectie en een
tweede tak in `FcmService._handleTap` voor `data['type'] == 'watch'`. Buiten scope gebleven:
zoekopdrachten aanmaken vanuit de chat-assistent/als AI-tool, historie van eerdere checkresultaten
en melden via Telegram/e-mail.
