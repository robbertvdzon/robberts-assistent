...
| `watches` | Sinds SF-1526: **zoekopdrachten** — periodiek een pagina checken op een AI-beoordeelde voorwaarde (bv. "waarschuw me zodra dit product weer op voorraad is"). `Watch`-model (`title`/`url`/`instruction`/`frequency`: `KANTOORUREN`/`DAGELIJKS`/`notifyOnFound`/`status`: `ONBEKEND`/`NIET_GEVONDEN`/`GEVONDEN`/`statusText`/`active`/`lastCheckedAt`); `WatchRepository`-poort met `FirestoreWatchRepository` (collectie `watches`)/`InMemoryWatchRepository`, gekozen door `WatchRepositoryConfig` (zelfde patroon als `reminders.ReminderRepositoryConfig`). `WatchesController`: `GET/POST /api/v1/watches`, `PUT/DELETE /api/v1/watches/{id}` (auth verplicht). `WatchesService.update()` reactiveert een gestopte watch (`active = true`, `status = ONBEKEND`, `lastCheckedAt = null`) — er is bewust geen apart activeer-knopje, "opnieuw aanpassen" via het bewerk-dialoog is de enige weg. `WatchPageFetcher` haalt de pagina op via `java.net.http.HttpClient` en heeft een eigen `htmlToPlainText()`-kopie (de variant in `assistant.ai.WindTools` is `internal` en niet herbruikbaar over modulegrenzen, bewaakt door `ModulithArchitectureTest`). `WatchAiConfig.watchChatClient`: losse, tool-loze `ChatClient` (patroon `briefing.BriefingAiConfig`), antwoordt in twee vaste regels; `WatchVerdict.parse()` parsed dit defensief ("NIET GEVONDEN" wordt eerst gecheckt, anders matcht de substring "GEVONDEN" ook een negatief antwoord) en valt bij een leeg/onverwacht antwoord (ook onder `RA_MOCK_AI`) terug op `ONBEKEND` zonder te crashen. `WatchScheduling.isDue()`: pure tijdlogica (`Europe/Amsterdam`) — `KANTOORUREN` (ma–vr 09–17u, elk uur) of `DAGELIJKS` (één keer per kalenderdag). `WatchScheduler` (`@Scheduled(fixedDelayString = "\${ra.watches.poll-interval-ms:300000}")`) filtert actieve + due watches, haalt op + laat beoordelen, en stuurt bij een transitie náár `GEVONDEN` (niet bij een poll die al `GEVONDEN` was) bij `notifyOnFound=true` precies één `PushService.sendToAll(..., data["type"]="watch")` en zet de watch daarna op `active = false`; een falende pagina-ophaal/AI-stap wordt per watch met `runCatching` geïsoleerd (andere watches en de volgende ronde blijven ongemoeid). Geen nieuwe `CouplingProbe`/secret — hergebruikt de bestaande `ChatModel`-bean en een kale `HttpClient`. Voedt de "Zoekopdrachten"-tab in de app (zie §6). |
...
- **`robberts_assistent/`** — bottom-navigatie met 6 tabs. Eerste tab is **"Upcoming"**
  ...
  gearchiveerde gesprekken alsnog zien. Sinds SF-1526 een vijfde tab **"Zoekopdrachten"**
  (`watches_screen.dart`, patroon `schedules_screen.dart`/`conversations_screen.dart`: CRUD-lijst
  + dialoog) op index 4 — "Meer" schuift op naar index 5, overige tabs/indices en de
  briefing-deep-link zijn ongewijzigd. Toont per zoekopdracht titel + statustekst (+ "gestopt"
  als `active=false`); FAB/tik-op-item opent een dialoog met titel/url/instructie, een
  frequentie-dropdown (kantooruren/dagelijks) en een pushmelding-schakelaar; verwijderen vraagt
  eerst een bevestiging. `ApiClient` heeft de bijbehorende `listWatches`/`createWatch`/
  `updateWatch`/`deleteWatch` tegen `/api/v1/watches` (zie `watches`-module in §4). Plus
  Koppelingen-, Nachtchecks- en **Geheugen**-schermen
...
Nieuw (SF-1526): nieuwe module `watches` + zesde app-tab **"Zoekopdrachten"** — Robbert kan
zoekopdrachten aanmaken die de assistent periodiek uitvoert (bv. "waarschuw me zodra dit product
weer op voorraad is op deze site"). Backend: `Watch`-model + `WatchRepository`-poort
(Firestore/in-memory, patroon `reminders`), CRUD via `WatchesController`
(`GET/POST /api/v1/watches`, `PUT/DELETE /api/v1/watches/{id}`), en één globale
`@Scheduled`-poller (`WatchScheduler`, `ra.watches.poll-interval-ms`, default 300000) die per
actieve watch een pure "aan de beurt?"-tijdlogica (`WatchScheduling`, `KANTOORUREN`: ma–vr 09–17u
elk uur, of `DAGELIJKS`: één keer per dag) toepast, de pagina ophaalt (`WatchPageFetcher`, eigen
`htmlToPlainText()`-kopie — de `WindTools`-variant is `internal` en niet herbruikbaar over
modulegrenzen) en laat beoordelen door een losse, tool-loze `watchChatClient`
(`WatchAiConfig`, patroon `briefing.BriefingAiConfig`) met defensieve antwoordparsing
(`WatchVerdict`, valt bij een onverwacht/leeg antwoord terug op `ONBEKEND`). Bij een transitie
náár `GEVONDEN` stuurt de poller bij `notifyOnFound=true` precies één
`PushService.sendToAll(..., data["type"]="watch")` en zet de watch op `active=false`; falen van
de pagina-ophaal- of AI-stap voor één watch wordt geïsoleerd (`runCatching`) en beïnvloedt andere
watches niet. Geen nieuwe `CouplingProbe`/secret nodig. Frontend: nieuwe tab op index 4 in
`robberts_assistent` (`watches_screen.dart`, "Meer" schuift op naar index 5) met een CRUD-lijst +
aanmaak/bewerk-dialoog (titel/url/instructie/frequentie/pushmelding-schakelaar); reactiveren van
een gestopte watch gebeurt impliciet via bewerken (geen apart activeer-knopje). Zie §4 (`watches`)
en §6 voor detail.
